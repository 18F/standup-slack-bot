    ...
    if (channel) {
      ...
      models.Standup.findAll({
        ...
        bot.startPrivateConversation({user: interviewUser}, function(response, convo){
          ...
          const interviewDay = moment().format('dddd');
          const interviewPrefix = `*[<#${interviewChannel}>, ${interviewDay}]*`;
          ...
          interviewSetup.then(function() {
            if(!singleSection || singleSection === 'yesterday') {
              convo.ask(interviewPrefix + ' What did you do yesterday?', function(response, conversation) {
            ...
            if(!singleSection || singleSection === 'today') {
              convo.ask(interviewPrefix + ' What are you doing today? (Use :pager: if you\'re available'+
            ...
            if(!singleSection || singleSection === 'blockers') {
              convo.ask(interviewPrefix + ' What are your blockers?'+pastBlockers, function(response, conversation) {
                if(!checkForExit(response, conversation)) {
                  if (!response.text.match(/^skip$/ig)) {
            ...
            if(!singleSection || singleSection === 'goal') {
              convo.ask(interviewPrefix + ' Any major goal for the day?', function(response, conversation) {
                if(!checkForExit(response, conversation)) {
                  if (!response.text.match(/^skip$/ig)) {
                    goal = response.text;
                  } else {
