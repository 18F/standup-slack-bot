...
module.exports = function start(botkit, bot) {
  if(process.env.PORT) {
    botkit.setupWebserver(process.env.PORT, function(err, server) {
      ...
      server.get('/oauth', (req, res) => {
        ...
        request(`https://slack.com/api/oauth.access?client_id=${process.env.SLACK_CLIENT_ID}&client_secret=${process.env.SLACK_CLIENT_SECRET}&code=${req.query.code}`, (err, res, body) => {
          console.log(body);
        });
        res.send('noooo!');
      });

      botkit.createWebhookEndpoints(botkit.webserver, process.env.SLACK_VERIFICATION_CODE);
    });
  } else {
