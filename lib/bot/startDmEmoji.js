...
function startDmEmoji(bot, message) {
  ...
  models.Channel.findOne({
    ...
    if (channel) {
      var latestReport = channel.latestReport;
                  ...
                  if (moment(now, 'hh:mm a Z').isBefore(moment(channelTime, 'hh:mm a Z'))) {
                    ...
                    models.Standup.findAll({
                      ...
                      var update = {
                        ts: latestReport,
                        channel: message.item.channel,
                        text: '<!here> Today\'s standup for <#'+channel.name+'>:',
                        attachments: [{
                          title: 'hello'
                        }],
                      };
                        ...
                        function(callback) {
                          bot.api.channels.info({'channel':message.item.channel}, function (err, response) {
                            if (err) {
                              ...
                              bot.say({
                                channel: message.item.channel,
                                text: 'Update! looks like '+standup.userRealName+
                                  ' chimed in. I\'ve updated the report: '+link
                              });
                            }
                          });
                          callback(null);
